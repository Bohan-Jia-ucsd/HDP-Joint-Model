...
Brier_HDP_sim<-list()
for (s in 1:100) {
Brier_HDP<-matrix(0,ncol = length(t_given),nrow = length(delta_t))
colnames(Brier_HDP)<-c(2,6,12,18)
rownames(Brier_HDP)<-c(1,2,3)
load(paste0("Hier_weibull_sim_Comp",s,".rdata"))
...
for (i in 1:nrow(dat_surv)) {
  ...
  pi_k_given_tY[1]<-pi_k_given_tY_propto(t=t_given[j],Y=Y_i$y,t_Y = Y_i$t,
                                         pi=pi[dat_surv$J[i],1],a=a,intercept=intercept[1],
                                         sigma=sigma[1],sigma_b = sigma_b[1],
                                         x=dat_surv$x[i],alpha=alpha,beta=beta[1],
                                         m=m[1],N=1000)
  pi_k_given_tY[2]<-pi_k_given_tY_propto(t=t_given[j],Y=Y_i$y,t_Y = Y_i$t,
                                         pi=pi[dat_surv$J[i],2],a=a,intercept=intercept[2],
                                         sigma=sigma[2],sigma_b = sigma_b[2],
                                         x=dat_surv$x[i],alpha=alpha,beta=beta[2],
                                         m=m[2],N=1000)
  pi_k_given_tY[3]<-pi_k_given_tY_propto(t=t_given[j],Y=Y_i$y,t_Y = Y_i$t,
                                         pi=pi[dat_surv$J[i],3],a=a,intercept=intercept[3],
                                         sigma=sigma[3],sigma_b = sigma_b[3],
                                         x=dat_surv$x[i],alpha=alpha,beta=beta[3],
                                         m=m[3],N=1000)
  pi_k_given_tY[4]<-pi_k_given_tY_propto(t=t_given[j],Y=Y_i$y,t_Y = Y_i$t,
                                         pi=pi[dat_surv$J[i],4],a=a,intercept=intercept[4],
                                         sigma=sigma[4],sigma_b = sigma_b[4],
                                         x=dat_surv$x[i],alpha=alpha,beta=beta[4],
                                         m=m[4],N=1000)
  ...
  P_u_given_tYk_store[1]<-P_u_given_tYk(t=t_given[j],u=t_given[j]+delta_t[l],Y=Y_i$y,t_Y = Y_i$t,
                                        a=a,intercept=intercept[1],sigma=sigma[1],sigma_b = sigma_b[1],
                                        x=dat_surv$x[i],alpha=alpha,beta=beta[1],m=m[1],
                                        N=1000)
  P_u_given_tYk_store[2]<-P_u_given_tYk(t=t_given[j],u=t_given[j]+delta_t[l],Y=Y_i$y,t_Y = Y_i$t,
                                        a=a,intercept=intercept[2],sigma=sigma[2],sigma_b = sigma_b[2],
                                        x=dat_surv$x[i],alpha=alpha,beta=beta[2],m=m[2],
                                        N=1000)
  P_u_given_tYk_store[3]<-P_u_given_tYk(t=t_given[j],u=t_given[j]+delta_t[l],Y=Y_i$y,t_Y = Y_i$t,
                                        a=a,intercept=intercept[3],sigma=sigma[3],sigma_b = sigma_b[3],
                                        x=dat_surv$x[i],alpha=alpha,beta=beta[3],m=m[3],
                                        N=1000)
  P_u_given_tYk_store[4]<-P_u_given_tYk(t=t_given[j],u=t_given[j]+delta_t[l],Y=Y_i$y,t_Y = Y_i$t,
                                        a=a,intercept=intercept[4],sigma=sigma[4],sigma_b = sigma_b[4],
                                        x=dat_surv$x[i],alpha=alpha,beta=beta[4],m=m[4],
                                        N=1000)
...
Brier_HDP[l,j]<-sum(I_survivor*
                      (I_T_u*(1-marg_P_u_given_tY)^2+
                                  (1-I_T_u)*dat_surv$status*(0-marg_P_u_given_tY)^2))/R_t
  }}
Brier_HDP_sim[[s]]<-Brier_HDP
}
save(Brier_HDP_sim,file = paste0("Brier_HDP_sim.rdata"))
