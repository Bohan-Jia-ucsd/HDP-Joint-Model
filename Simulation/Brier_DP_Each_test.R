...
t_given<-c(2,6,12,18)
delta_t<-c(1,2,3)
Brier_DP_Each_sim<-list()
for (s in 1:100) {
Brier_DP_Each<-matrix(0,ncol = length(t_given),nrow = length(delta_t))
colnames(Brier_DP_Each)<-c(2,6,12,18)
rownames(Brier_DP_Each)<-c(1,2,3)
...
Brier_DP_Each[l,j]<-sum(I_survivor*
                      (I_T_u*(1-marg_P_u_given_tY)^2+
                                  (1-I_T_u)*dat_surv$status*(0-marg_P_u_given_tY)^2))/R_t
  }}
Brier_DP_Each_sim[[s]]<-Brier_DP_Each
}
save(Brier_DP_Each_sim,file = paste0("Brier_DP_Each_sim.rdata"))
