...
t_given<-c(2,6,12,18)
delta_t<-c(1,2,3)
Brier_DP_All_sim<-list()
for (s in 1:100) {
Brier_DP_All<-matrix(0,ncol = length(t_given),nrow = length(delta_t))
colnames(Brier_DP_All)<-c(2,6,12,18)
rownames(Brier_DP_All)<-c(1,2,3)
load(paste0("DP_all_sim_Comp",s,".rdata"))
...
for (i in 1:nrow(dat_surv)) {
  ...
  pi_k_given_tY[1]<-pi_k_given_tY_propto(t=t_given[j],Y=Y_i$y,t_Y = Y_i$t,
                                         pi=pi[1],a=a,intercept=intercept[1],
  ...
  pi_k_given_tY[2]<-pi_k_given_tY_propto(t=t_given[j],Y=Y_i$y,t_Y = Y_i$t,
                                         pi=pi[2],a=a,intercept=intercept[2],
  ...
  pi_k_given_tY[3]<-pi_k_given_tY_propto(t=t_given[j],Y=Y_i$y,t_Y = Y_i$t,
                                         pi=pi[3],a=a,intercept=intercept[3],
  ...
  pi_k_given_tY[4]<-pi_k_given_tY_propto(t=t_given[j],Y=Y_i$y,t_Y = Y_i$t,
                                         pi=pi[4],a=a,intercept=intercept[4],
...
Brier_DP_All[l,j]<-sum(I_survivor*
                      (I_T_u*(1-marg_P_u_given_tY)^2+
                                  (1-I_T_u)*dat_surv$status*(0-marg_P_u_given_tY)^2))/R_t
  }}
Brier_DP_All_sim[[s]]<-Brier_DP_All
}
save(Brier_DP_All_sim,file = paste0("Brier_DP_All_sim.rdata"))
