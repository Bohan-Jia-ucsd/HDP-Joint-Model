...
dat_surv<-dat_surv%>%filter(J==1)
dat_lmm<-dat_lmm%>%filter(J==1)
n=nrow(dat_surv)
dat_surv<-dat_surv%>%mutate(id=1:n)
remained_id<-dat_lmm%>%group_by(id)%>%summarise(n=n())
id_new<-c()
for (i in 1:n) {
...
jags.m <- jags( model.file= "DP_each1_joint.txt", 
                data=data_test, 
...
save(jags.m, file = paste0("DP_Each1_sim_Comp",SEED.START,".rdata"))
